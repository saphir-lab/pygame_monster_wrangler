...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if keys[pygame.K_LEFT] and self.rect.left > self.play_zone.left:
            self.rect.x -= self.velocity
        if keys[pygame.K_RIGHT] and self.rect.right < self.play_zone.width :
            self.rect.x += self.velocity
        if keys[pygame.K_UP] and self.rect.top > self.play_zone.top:
            self.rect.y -= self.velocity
        if keys[pygame.K_DOWN] and self.rect.bottom < self.play_zone.bottom:
            self.rect.y += self.velocity
