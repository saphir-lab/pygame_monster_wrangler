...
class Dashboard():
    ...
    def draw(self, lives:int, warps:int, target_monster_image:pygame.Surface):
        ...
        lives_text = self.font.render("Lives: " + str(lives), True, self.font_color )
        lives_rect = lives_text.get_rect()
        lives_rect.topleft = (self.screen_rect.left + text_margin, 35)

        round_text = self.font.render("Current Round: " + str(self.round_number), True, self.font_color )
        round_rect = round_text.get_rect()
        round_rect.topleft = (self.screen_rect.left + text_margin, 65)

        time_text = self.font.render("Round Time: " + str(self.round_time), True, self.font_color )
        time_rect = time_text.get_rect()
        time_rect.topright = (self.screen_rect.right - text_margin, 5)

        warp_text = self.font.render("Warps: " + str(warps), True, self.font_color )
